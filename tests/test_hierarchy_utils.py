...
def test_get_in_usage():
    ...
    assert get_in_hp("/nba/*/details/*/who", teams) == [
        ["bill", "ted", "fred"],
        ["ken"],
    ]

    # deep nesting... notice how the structure of the output mirrors the input.
    assert get_in_hp("/nba/*/details/*/pos/*/", teams) == [
        [["r", "c", "l"], ["d"], None],
        [["c"]],
    ]
    ...
    assert flatten_recur(get_in_hp("/nba/*/details/*/pos/*/", teams)) == [
        "r",
        "c",
        "l",
        "d",
...
def test_get_in_hp_stars_single():
    "test for wildcards in the spec identifier.  Should return a list of results."
    assert get_in_hp("/ExtraData/*/Name", more_fake_data) == [
        "LikesCake",
        "HasTwin",
        "EnjoysTwin",
        "FavCity",
    ...
    assert get_in_hp("/TeamPlayers/*/Name/GivenName", more_fake_data) == [
        "person",
        "person",
    ]
    assert get_in_hp("/TeamPlayers/*/Name/Surname", more_fake_data) == ["one", "two"]
    assert get_in_hp("/TeamPlayers/*/DOB", more_fake_data) == [
        "1984-01-01",
        "1901-01-01",
    ]
...
def test_get_in_hp_stars_multiple():
    "test for wildcards in the spec identifier.  Should return a list of results."
    assert get_in_hp("/TeamPlayers/*/jerseyDetails/*/JerseyNumber", more_fake_data) == [
        ["#1", "1"],
        ["ID2"],
    ]
    assert get_in_hp("/TeamPlayers/*/jerseyDetails/*/Size", more_fake_data) == [
        ["Large", "Small"],
        ["Large"],
    ]
...
def test_my_assoc_in_coll():
    ...
    assert assoc_in_coll(td1, ["nhl", 0, "players"], 1976) == {
        ...
        "compile-secret": "SECRET!",
        "nhl": [
            {"team": "stars", "players": 1976, "pos": ["l", "r", "c"]},
            {"team": "bruins", "players": 30},
            {"team": "preds", "players": 90},
        ],
        "nba": [
    ...
    assert (
        assoc_in_coll(nba_teams, ["nba", 0, "details", 1, "who"], "new coach!")
        == new_coach
    )
...
def test_update_in_hp():
    ...
    assert update_in_hp(
        {"nhl": [{"team": "stars", "players": 10}, {"team": "preds", "players": 90}]},
        "/nhl/0/players/",
        my_fn,
    ) == {
        "nhl": [{"team": "stars", "players": 11}, {"team": "preds", "players": 90}]
    }, "Test single Path in list update."
    ...
    assert update_in_hp(
        ...
        "nhl": [{"team": "stars", "players": 11}, {"team": "preds", "players": 91}]
    }, "Test Wildcard path update."

    assert update_in_hp(
        {
            "nhl": [
                {"team": "stars", "players": 10},
                {"team": "bruins"},
                {"team": "preds", "players": 90},
            ]
        },
        ...
        "nhl": [
            {"team": "stars", "players": 11},
            {"team": "bruins", "players": 1},
            {"team": "preds", "players": 91},
        ]
    }, "Test Wildcard path update."
